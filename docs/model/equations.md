...
Positive variables are log-transformed: $L = \\ln(1 + x)$, and bounded variables use the logit transform: $\\eta = \\ln(\\frac{p}{1-p})$.
...
Bilateral flow from region $j$ to region $i$:
...
See [Nonlinear Thresholds](thresholds.md) for expanded discussion of each mechanism.
